...
final class PcmMonoAudioEngine implements AutoCloseable {
    ...
    private PcmMonoAudioEngine(PcmMonoSource source, SourceDataLine line, String threadName) {
        this.source = source;
        this.line = line;
    ...
    static PcmMonoAudioEngine start(PcmMonoSource source, String threadName) throws LineUnavailableException {
        ...
        PcmMonoAudioEngine engine = new PcmMonoAudioEngine(source, line, threadName);
        engine.worker.start();
        return engine;
    }
    ...
    private void runLoop() {
        ...
        while (running.get()) {
            int copied = source.drainAudio(chunk, 0, chunk.length);
            if (copied > 0) {
